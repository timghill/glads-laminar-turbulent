"""
Plot flux for turbulent, laminar, and transition models
...
cases = [201, 203, 204]
n_cases = 3
figname = 'qs_quiver.png'

tslices = [int(365*(1+5/12)), int(365*(1 + 7/12))]
tlabels = [5, 7]
n_times = len(tslices)

# Define the figure
fig = plt.figure(figsize=(8, 5))
gs = GridSpec(n_cases+1, n_times,
    wspace=0.1, hspace=0.05,
    left=0.1, right=0.95, top=0.9, bottom=0.1,
    height_ratios=([8] + n_cases*[100]),
...
for ii in range(n_cases):
    ...
    for jj in range(n_times):
        ax = axs[ii, jj]
        fcolor = ax.tripcolor(mtri, np.log10(np.abs(qs[:, jj])), cmap=cmocean.cm.tempo, vmin=-6, vmax=0)
        # fcolor = ax.tricontourf(mtri, np.log10(np.abs(qs[:, jj])), cmap=cmocean.cm.tempo, levels=np.linspace(-6, 0, 12))
        # fcolor = ax.tripcolor(mtri, qx[:, jj], cmap=cmocean.cm.balance, vmin=-1e-3, vmax=1e-3)
        qx_scale = np.log10(np.abs(qx)) + 6
        qy_scale = np.log10(np.abs(qy)) + 6
        angle = 180/np.pi * np.arctan2(qy, qx)
        ax.quiver(elements[::10, 0]/1e3, elements[::10, 1]/1e3, qx_scale[::10, jj], qy_scale[::10, jj], angles=angle[::10, jj])
        ax.set_aspect('equal')
        ax.set_xlim([0, 100])
        ax.set_ylim([0, 25])
        ...
        if ii==0:
            ax.text(95, 23, 't = %s months' % tlabels[jj], ha='right', color='w',
        ...
        # axs_scatter[jj].scatter(elements[:, 0]/1e3, np.log10(np.abs(qs[:, jj])), 5, alpha=0.5, color=colors[ii],
        #     label=labels[ii])
        # axs_scatter[jj].set_ylim([-6, -1])
...
# axs_scatter[0].set_ylabel(r'$\log_{10}|\mathbf{q}_{\rm{s}}|$')
...
cax1.set_xlabel(r'$\log_{10}|\mathbf{q}_{\rm{s}}|$')
