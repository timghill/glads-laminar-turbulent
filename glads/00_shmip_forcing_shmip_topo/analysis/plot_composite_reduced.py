...
global_gs = gridspec.GridSpec(1, 3, wspace=0.05, hspace=0.2, 
    width_ratios = (100, 30, 115), left=0.1, right=0.92, bottom=0.1)
...
for ii in range(n_cases):
    ...
    print(ff.shape)
    ax_scatter.scatter(nodes[:, 0]/1e3, ff[:, tslice], 1, alpha=0.5, color=colors[ii],
        label=labels[ii])
    ...
    for j, xb in enumerate(x_bands):
        ...
        timeax_right = timeax.twinx()
        # temp_interp = np.interp(tt, tt_days/365, temp_sl, left=0, right=0)
        temp_interp = temp_fun(tt*86400*365)
        temp_interp[temp_interp<0] = 0
        timeax_right.plot(tt, temp_interp, color='k', label='Temperature', zorder=0, linewidth=0.5)
        timeax_right.set_ylim([0, 12])
        timeax_right.set_ylabel('Temp ($^\circ$C)')
...
cax1 = fig.add_subplot(gs_maps[1:4, 1])
...
for j, xb in enumerate(x_bands):
    ...
    axi.set_xlim([1, 2])
    axi.set_ylim([0, 1.5])
    axi.set_xticks([1, 1.25, 1.5, 1.75, 2])
    axi.grid()
