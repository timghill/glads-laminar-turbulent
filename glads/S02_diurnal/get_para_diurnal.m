...
pm.model_run_descript = 'Run diurnal';
pm.save_filename = [pm.dir.model_save, filename];

%% Time
pt.day = 86400;
pt.start = 100*pp.year;
pt.end   = pt.start + 5*pt.day;  % end time
pt.out_t = pt.start : pt.day/24 : pt.end;
...
pin.source_term_c = make_anon_fn('@(time) double(source_moulin_shmip_adj_diurnal(time, pin, dmesh, ii_moulin, catchmap));', pin, dmesh, ii_moulin, catchmap);
