...
for ii in range(n_cases):
    ...
    hs = out['h_sheet'][tslices, :].data.T

    mtri = Triangulation(nodes[:, 0]/1e3, nodes[:, 1]/1e3, connect)

    for jj in range(n_times):
        ax = axs[ii, jj]
        fcolor = ax.tripcolor(mtri, hs[:, jj], cmap=palettes.get_cmap('blue-1').reversed(), vmin=0, vmax=0.5)
        ax.set_aspect('equal')
        ax.set_xlim([0, 100])
        ax.set_ylim([0, 25])
        ...
        if ii==0:
            ax.text(95, 23, 't = %s a' % tlabels[jj], ha='right', color='k',
                fontweight='bold', va='top')
        if jj==1:
            ax.text(95, 2, labels[ii], ha='right', color='k', fontweight='bold')
        ...
        axs_scatter[jj].scatter(nodes[:, 0]/1e3, hs[:, jj], 5, alpha=0.5, color=colors[ii],
            label=labels[ii])
        axs_scatter[jj].set_ylim([0, 0.6])
...
axs_scatter[0].set_ylabel(r'$h_{\rm{b}}~(\rm{m})$')
...
cax1.set_xlabel(r'$h_{\rm{b}}~(\rm{m})$')
